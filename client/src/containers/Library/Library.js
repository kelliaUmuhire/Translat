...
import { getLibrary, removeFromLib } from "../../store/actions/booksAction";
...
class Library extends Component {
  ...
  removeBook = (bookId) => {
    axios
      .post(`/api/library/removebook/${bookId}`)
      .then((res) => {
        if (res.data.success) {
          this.props.removeinlib(bookId);
          let temp = [...this.state.tempBooks];
          temp = temp.filter((b) => b._id !== bookId);
          this.setState({ tempBooks: temp });
        }
      })
      .catch((err) => console.log(err));
...
const mapDispatchToProps = (dispatch) => ({
  ...
  removeinlib: (bookId) => dispatch(removeFromLib(bookId)),
});
